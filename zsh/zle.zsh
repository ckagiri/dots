...
# Initialization wrapped with xtrace suppression to keep sourcing clean.
setopt localoptions no_xtrace
typeset -g _cursor_magenta _cursor_cyan _cursor_cmd_shape _cursor_ins_shape _use_vte _can_osc12 _has_terminfo_shape

# Detect VTE-based terminals (GNOME Terminal, Tilix, etc.) which set VTE_VERSION.
if [[ -n ${VTE_VERSION} || -n ${GNOME_TERMINAL_SCREEN} || -n ${GNOME_TERMINAL_SERVICE} ]]; then
  _use_vte=1
else
  # Walk up process tree (limited depth) to detect gnome-terminal / tilix even if env vars lost.
  function _probe_vte_parents() {
    setopt localoptions no_xtrace
    local pid=$$ depth=0 ppid comm
    while (( depth < 8 )); do
      ppid=$(ps -o ppid= -p "$pid" 2>/dev/null | tr -d '[:space:]') || break
      [[ -z $ppid || $ppid == 1 ]] && break
      comm=$(ps -o comm= -p "$ppid" 2>/dev/null)
      case $comm in
        gnome-terminal*|gnome-terminal-server|tilix|x-terminal-emulator)
          _use_vte=1
          return
          ;;
      esac
...
# Determine if OSC 12 cursor color likely works. Generally xterm/rxvt/konsole/kitty.
if [[ $TERM == xterm* || $TERM == rxvt* || $TERM == konsole* || $TERM == screen.xterm* || $TERM == screen* || $TERM == tmux* || $TERM == kitty* || $TERM == alacritty* || $TERM == foot* ]]; then
  [[ -z $_use_vte ]] && _can_osc12=1
fi

# Force-disable OSC 12 if VTE detected (GNOME Terminal ignores it; avoid noise).
if [[ -n $_use_vte ]]; then
  _can_osc12=
fi

# Terminfo cursor-shape capability: modern ncurses adds Ss/Se. Test via infocmp.
if infocmp -1 2>/dev/null | grep -q '^Ss='; then
  _has_terminfo_shape=1
fi
...
if [[ "$OSTYPE" == darwin* ]]; then
  ...
  if [[ -n $_can_osc12 ]]; then
    # Prefer hex form (#RRGGBB) for OSC 12.
    _cursor_magenta=$'\e]12;#d33682\a'
    _cursor_cyan=$'\e]12;#2aa198\a'
    if [[ -n ${TMUX} ]]; then
      _cursor_magenta=$'\ePtmux;\e'"${_cursor_magenta}"$'\e\\'
      _cursor_cyan=$'\ePtmux;\e'"${_cursor_cyan}"$'\e\\'
    fi
  fi

  # Cursor shape fallback (DECSCUSR) or terminfo if present.
  # Shapes: 2 = steady block (command mode), 6 = steady bar (insert).
  if [[ -n ${TMUX} ]]; then
    # tmux generally passes through DECSCUSR; wrapping not required, but keep simple.
    _cursor_cmd_shape=$'\e[2 q'
    _cursor_ins_shape=$'\e[6 q'
  else
    _cursor_cmd_shape=$'\e[2 q'
    _cursor_ins_shape=$'\e[6 q'
  fi
...
# If terminfo shape exists, define helper to use it (overrides DECSCUSR for portability).
if [[ -n $_has_terminfo_shape ]]; then
  function _set_shape_cmd() { tput Ss 2 0 0 >/dev/null 2>&1; }
  function _set_shape_ins() { tput Ss 6 0 0 >/dev/null 2>&1; }
else
  function _set_shape_cmd() { print -n -- "${_cursor_cmd_shape}"; }
  function _set_shape_ins() { print -n -- "${_cursor_ins_shape}"; }
fi

function zle-keymap-select {
  if [[ $KEYMAP = vicmd ]]; then
    if [[ -n $_can_osc12 && -n ${_cursor_magenta} ]]; then
      print -n -- "${_cursor_magenta}"
    else
      _set_shape_cmd
    fi
  else
    if [[ -n $_can_osc12 && -n ${_cursor_cyan} ]]; then
...
# Do not emit any cursor sequence on source; handled lazily by widgets.
